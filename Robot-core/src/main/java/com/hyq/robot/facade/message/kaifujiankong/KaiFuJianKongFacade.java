...
/**
 * @author nanke
 * @date 2021/1/6 下午2:41
 * 致终于来到这里的勇敢的人:
 * 永远不要放弃！永远不要对自己失望！永远不要逃走辜负了自己。
 * 永远不要哭啼！永远不要说再见！永远不要说慌来伤害目己。
 */
@Component
public class KaiFuJianKongFacade implements MessageFacade {
    ...
    public void execute(Contact sender, Contact group, Message message) {
        ...
        if (!CollectionUtils.isEmpty(subscribeDOS)) {
            SendHelper.sendSing(group,at.plus("已订阅"));
            return ;
        }
        // 订阅
        OpenServiceSubscribeDO insertDO = new OpenServiceSubscribeDO();
        insertDO.setQq(sender.getId());
        insertDO.setGroupId(group.getId());
        insertDO.setMainId(mainId);
        openServiceSubscribeDAO.insertSelective(insertDO);
        SendHelper.sendSing(group,at.plus("订阅成功"));
    }
}
